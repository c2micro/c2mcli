...
func UserBeaconCd(args ...object.Object) (object.Object, error) {
	if len(args) != 2 {
	...
	if err := BackendBeaconCd(uint32(id.GetValue().(int64)), path.GetValue().(string)); err != nil {
		return nil, err
	}
	return object.NewNull(), nil
}

func BackendBeaconCd(id uint32, path string) error {
	cap := defaults.CAP_CD
	...
	return service.NewTask(id, &operatorv1.NewTaskRequest{
		Cap: uint32(cap),
		Args: &operatorv1.NewTaskRequest_Cd{
			Cd: &commonv1.CapCd{
				Path: path,
			},
		},
