...
func UserBeaconCp(args ...object.Object) (object.Object, error) {
	if len(args) != 3 {
	...
	if err := BackendBeaconCp(uint32(id.GetValue().(int64)), src.GetValue().(string), dst.GetValue().(string)); err != nil {
		return nil, err
	}
	return object.NewNull(), nil
}

func BackendBeaconCp(id uint32, src, dst string) error {
	cap := defaults.CAP_CP
	...
	return service.NewTask(id, &operatorv1.NewTaskRequest{
		Cap: uint32(cap),
		Args: &operatorv1.NewTaskRequest_Cp{
			Cp: &commonv1.CapCp{
				Src: src,
				Dst: dst,
			},
