...
func UserBeaconPs(args ...object.Object) (object.Object, error) {
	if len(args) != 1 {
	...
	if err := BackendBeaconPs(uint32(id.GetValue().(int64))); err != nil {
		return nil, err
	}
	return object.NewNull(), nil
}

func BackendBeaconPs(id uint32) error {
	cap := defaults.CAP_PS
	...
	return service.NewTask(id, &operatorv1.NewTaskRequest{
		Cap: uint32(cap),
		Args: &operatorv1.NewTaskRequest_Ps{
			Ps: &commonv1.CapPs{},
		},
	})
}
