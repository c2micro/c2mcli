...
func UserBeaconDownload(args ...object.Object) (object.Object, error) {
	if len(args) != 2 {
	...
	if err := BackendBeaconDownload(uint32(id.GetValue().(int64)), path.GetValue().(string)); err != nil {
		return nil, err
	}
	return object.NewNull(), nil
}

func BackendBeaconDownload(id uint32, path string) error {
	cap := defaults.CAP_DOWNLOAD
	...
	return service.NewTask(id, &operatorv1.NewTaskRequest{
		Cap: uint32(cap),
		Args: &operatorv1.NewTaskRequest_Download{
